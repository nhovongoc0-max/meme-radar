...
本地运行的多链 Meme 候选雷达。AVE 负责发现、池行情与 K 线；GoPlus 和 DexScreener 在已支持链上做风险与行情交叉核对。当前公开版本为 **v0.1.9**。
...
- 提供五条已核验入口，可选择 1–3 条链轮询。BSC、Solana、Base、以太坊按 AVE 官方接口接入；Robinhood 以本机真实成功响应为准。Arc、Stable 尚无成功记录且缺少二次安全覆盖，暂不作为可用链展示。
...
- [Windows x64 一键便携版](https://github.com/nhovongoc0-max/meme-radar/releases/download/v0.1.9/MemeRadar-OpenSource-Windows-x64-0.1.9.zip)
- [macOS 版](https://github.com/nhovongoc0-max/meme-radar/releases/download/v0.1.9/MemeRadar-OpenSource-macOS-0.1.9.zip)
- [SHA-256 校验文件](https://github.com/nhovongoc0-max/meme-radar/releases/download/v0.1.9/SHA256SUMS-0.1.9.txt)
...
- HTTP 服务只监听本机回环地址。
- 设置接口仅处理本机扫描配置、收藏备注、AVE Key 与显式触发的更新；无签名或下单接口。
...
顶部 **一键更新** 检查正式版本，展示目标版本后确认更新。下载包经固定仓库、SHA-256 和当前安装文件校验，保留配置与记录，启动异常时尝试回滚。Git 源码目录或本机未发布改动不会被自动覆盖；只有通过校验的正式版本才会进入更新流程。

适用于 Windows x64 便携版及依赖锁未变化的 macOS 包；依赖发生变化时停止自动安装。Windows 流程已做离线测试，尚待真机验收。备份不会自动删除。
...
- 收藏与备注保存在本机 `state/preferences.json`，最多 50 个收藏、500 条备注。收藏币掉出发现范围后继续复查风险，不因此重新成为通过候选。关闭某条链的扫描后不再执行该链的风险复查或额外历史补查，已有记录保留。
...
提醒覆盖已启用的扫描链，仅对最近 15 分钟内首次通过快速筛选、行情仍有效且未命中已知风险排除的新候选播放。首次打开、重新开启和断线恢复不追播旧候选。每批合并一句，最多每分钟一次，同一链＋合约地址 7 天去重；换页、行情刷新或播报被打断都不会让同一币重复播报。关闭与音量变化同步到同源其他标签页，其他页面不能替当前页面解锁声音。台词是提醒语，不是质量评级、诈骗鉴定或收益承诺。

已知流动性不足8,000美元、DEV持仓超过1%、买卖税任一超过5%或相差超过2个百分点、明确近5分钟零成交，均不进入发现候选；未知值不会被包装成已确认安全。“已卖出”标签不能覆盖DEV的实际持仓数值，也不能把缺失的持仓当成零。
...
窗口先排除已知貔貅、刷量、高风险、创建不足5分钟和基础流动性不足的记录；未知风险仍明确为未核验。展示范围为市值1万–15万美元，可按2万–8万美元优先、5分钟成交额或最近出现排序。最近出现指首次进入当前有效候选窗口，不代表刚发币；候选超过15分钟自动退出，重新上榜也不会反复播报。首屏不伪造“新币”，榜单缺失也不填充演示数据。
